...
def main(stdscr):
    ...
    saved = False  # Add this before the while loop
    ...
    while True:
        ...
        status = (
            f"<< Havim >>    File: {filename} | Ln {cursor.row+1}, Col {cursor.col+1} | "
        ...
        k = stdscr.getkey()
        saved = False  # Reset saved flag on every keypress
        if k == "\x11":  # Ctrl+Q
            sys.exit(0)
        elif k == "\x13":  # Ctrl+S
            with open(args.filename, "w", encoding="utf-8") as f:
                f.write("\n".join(buffer.lines))
            saved = True  # Set flag when saved
            # Show "file saved" for 3 seconds
            status = (
                f"Ln {cursor.row+1}, Col {cursor.col+1} | "
            ...
            stdscr.refresh()
            curses.napms(3000)
            saved = False  # Remove "file saved" after 3 seconds
        elif k == "KEY_LEFT":
